...
def simulate_scenario(sigmas: dict[str, float]) -> dict:
    """σ 단위 조정으로 TEX 2025 반사실 시즌 시뮬.

    피처별 σ(표준편차 배율)를 받아 ML 잔차 앙상블로 예상 승수를 산출한다.
    양수=개선, 음수=악화 (lower-better 피처도 양수=개선으로 통일).

    조정 가능 피처: sv_pct, SV_pg, onerun_wp, xi_wp, home_away_diff, WHIP,
    k_bb, K9, BB9, HR9, ir_pct, babip_against, go_ao, sb_pct, era_fip_diff.
    (ERA, OPS, rs_per_g는 pyth_W 채널 중복으로 제외 권장)

    Args:
        sigmas: {'K9': 0.3, 'BB9': 0.4} 같은 dict.

    Returns:
        예상 승수, delta(잔차 변화), 베이스라인 승수, 피처별 조정 내역.
    """
    return simulation_core.simulate_scenario(sigmas)
...
def swap_team_pitching(team: str) -> dict:
    ...
    K9/BB9/HR9/ERA 차이를 σ 단위로 변환해 simulate_scenario 호출.
    ...
    sim_result = simulation_core.simulate_scenario(sigmas)
