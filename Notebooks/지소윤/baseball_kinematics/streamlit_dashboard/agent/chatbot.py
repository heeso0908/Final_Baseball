...
def get_agent() -> Agent:
    ...
    def swap_team_pitching(team: str) -> dict:
        ...
        K/9, BB/9, HR/9, BABIP를 σ로 변환해 simulate_scenario 호출.
        """
    ...
    with st.chat_message("assistant"):
        with st.spinner("분석 중..."):
            try:
                ...
                st.error(f"Agent 호출 실패: {e}")
                return

        st.markdown(result.output)

        # 도구 호출 추출 (transparency)
        tool_calls = []
